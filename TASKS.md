...
| `CORE-3` | P0 | 2 | `NEXT` | `Zaki` | Логирование | Подключить `slf4j` + `logback`, убрать `System.out` из bootstrap-логики по мере роста приложения; заменить `java.util.logging` в `PeerRegistry` на slf4j. | Консольные и файловые логи работают, уровень задается из конфига/env; `PeerRegistry` логирует через slf4j. |
